...
export default function EditBloodBank({item}) {
    ...
    const onSubmit = async (data) => {
        ...
        try {
          const response = await axios.put("http://localhost:3000/bloodBank/updateBloodBank/"+item.id, formData);
  ...
  export async function getServerSideProps(context) {
    const id=context.params.id;
       ...
       const response = await axios.get('http://localhost:3000/bloodBank/getBloodBank/'+id);
