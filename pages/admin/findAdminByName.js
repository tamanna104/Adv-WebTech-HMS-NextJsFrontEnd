...
export default function findAdminByName({ data }) {
    ...
    return (
      <>
          ...
          <form className="w-full max-w-sm" onSubmit={handleFormSubmit}>
            <div className="flex items-center border-b border-b-2 border-teal-500 py-2">
              <input value={inputValue} onChange={handleInputChange} 
                ...
                type="text" placeholder="Find By Name" aria-label="ID" />
        ...
        <div class="flex justify-center">
          <ul>
            {/* {data.status ? 
              <h4 class="text-red-600 text-lg">{data.status}</h4> : 
              data && data.map(item => (
              <li key={item.id}>
  ...
  export async function getServerSideProps({ query }) {
    ...
    try {
      const response = await axios.get('http://localhost:3000/admin/getAdminByName/'+inputValue);
